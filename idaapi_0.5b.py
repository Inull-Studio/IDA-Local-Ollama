...
# 定义模型
model_name_1 = 'qwen2.5:0.5b'  # 第一个智能体使用的模型
model_name_2 = 'qwen2.5:0.5b'  # 第二个智能体使用的模型
            ...
            if text:
                ...
                for i, line in enumerate(lines[:-1]):
                    if "<|" in line:
                        break
                    if line.endswith('\n'):
                        print(f":: {line.rstrip()}")  # 输出行
                        optimized_code_lines.append(line.rstrip())
                # 如果optimized_code_lines行内有重复元素超过10次，则break，但是要审查重复元素里面要忽略哪些包含字符或者数字的重复元素
                # if len(optimized_code_lines) > 10 and len(set(optimized_code_lines)) < len(optimized_code_lines) - 10:
                #     # # 检查重复的元素里面有哪些包含字符或者数字的重复元素
                #     repeat_list = []
                #     for item in set(optimized_code_lines):
                #         if optimized_code_lines.count(item) > 10 and re.search(r'[0-9a-zA-Z]', item):
                #             repeat_list.append(item)
                #     # 如果重复的元素里面有包含字符或者数字的元素，则break
                #     if repeat_list:
                #         print(f"### 重复元素超过10次")
                #         break
