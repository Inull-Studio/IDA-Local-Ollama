...
'''
    ...
    try:
        response_stream = client_1.chat(model=model_name_1, messages=[user_message], stream=True, options={'max_turns': 1,'temperature': 0,'top_p': 0.7})
        buffer = ""
...
''')
        for category, items in [
            ...
            for item in items:
                ...
                try:
                    # print(f"[DEBUG] {query}")
                    response = client_2.chat(model=model_name_2, messages=[{'role': 'user', 'content': query}], stream=False, options={'max_turns': 1,'temperature': 0.2})
